...
public final class SearchStore {
    ...
    public init() {
        ...
        if sqlite3_open(dbURL.path, &db) != SQLITE_OK {
            MurmurLogger.shared.log("search db open failed: \(dbURL.path)")
            db = nil
            return
        }
    ...
    public func insertOCRText(timestamp: Date, chunkID: Int, text: String) {
    ...
    public func insertTranscript(timestamp: Date, chunkID: Int, text: String) {
        queue.async { [weak self] in
            self?.execute(
    ...
    public func insertMetadata(timestamp: Date, appName: String?, windowTitle: String?, url: String?) {
        queue.async { [weak self] in
            self?.execute(
    ...
    public func metadata(from start: Date, to end: Date) -> [(timestamp: Date, appName: String?, windowTitle: String?)] {
    ...
    public func pruneOlderThan(_ date: Date) {
    ...
    private func execute(_ sql: String, bindings: [SQLBinding] = []) {
        ...
        if bindings.isEmpty {
            ...
            if sqlite3_exec(db, sql, nil, nil, &errMsg) != SQLITE_OK {
                ...
                MurmurLogger.shared.log("sql error: \(msg)")
                sqlite3_free(errMsg)
            }
        } else {
            var stmt: OpaquePointer?
            guard sqlite3_prepare_v2(db, sql, -1, &stmt, nil) == SQLITE_OK else {
                MurmurLogger.shared.log("sql prepare error: \(String(cString: sqlite3_errmsg(db)))")
                return
            }
            defer { sqlite3_finalize(stmt) }
            ...
            if sqlite3_step(stmt) != SQLITE_DONE {
                MurmurLogger.shared.log("sql step error: \(String(cString: sqlite3_errmsg(db)))")
            }
        }
    }
}
